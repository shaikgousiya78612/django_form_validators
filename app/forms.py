...
def check_for_g(value):
    if value[0].upper()=='G':
        raise forms.ValidationError('should not start with g')
def check_for_len(v):
    if len(v)<=5:
...
class NameForm(forms.Form):
    name=forms.CharField(max_length=100,validators=[check_for_g,validators.MaxLengthValidator(10)])
    fathername=forms.CharField(max_length=100,validators=[check_for_g])
